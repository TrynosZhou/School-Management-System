...
@Component({
  selector: 'app-tt-teachers',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="wrap">
      <h2>Timetable • Teachers</h2>
      <div class="panel">
        <div class="inline">
          <input [(ngModel)]="name" placeholder="Teacher name"/>
          <input [(ngModel)]="email" placeholder="Email (optional)"/>
        ...
        <div class="list" *ngIf="rows().length">
          <div class="item" *ngFor="let t of rows(); let i = index">
            <div class="name">{{ t.name }}</div>
            <div class="sub">{{ t.email || '' }}</div>
            <button type="button" class="danger" (click)="remove(i)">Remove</button>
          </div>
        </div>
        <div class="muted" *ngIf="!rows().length">No teachers yet</div>
      </div>
    </div>
  `,
  styles: [`
...
export class TtTeachersComponent {
  private http = inject(HttpClient);
  rows = signal<Teacher[]>([]);
  name = '';
  email = '';

  load(){ this.http.get<any[]>(`/api/teachers`).subscribe({ next: a => this.rows.set((a||[]).map(r=>({ id: r.id, name: `${r.firstName||''} ${r.lastName||''}`.trim() || r.name || r.id, email: r.email }))), error: () => {} }); }
  add(){ const id = crypto.randomUUID(); const t: Teacher = { id, name: this.name.trim(), email: this.email.trim() || undefined }; this.rows.set([...this.rows(), t]); this.name=''; this.email=''; }
